...
namespace SmartClickCore.API.Controllers.PSP
{
    ...
    public class EntitiesController : ControllerBase
    {
        ...
        // M�todo corregido para buscar usuario por UAT
        ...
        public async Task<IActionResult> CrearUsuario([FromBody] CreateUserEntidadRequestDTO request)
        {
            try
            {
                ...
                var mensaje = _pspService.IsTestMode() 
                    ? "?? SIMULACI�N: Usuario creado (modo prueba)"
                    ...
                    if (pspResponseToken != null && !string.IsNullOrEmpty(pspResponseToken.access_token))
                    {
                        ...
                        if (pspResponseSelf.Success)
                        {
                            ...
                            response.Mensaje += " | " + ( _pspService.IsTestMode() 
                                ? "?? SIMULACI�N: Entidad creada mediante SelfRegistration (modo prueba)"
                            ...
                            if (pspResponseFiles.Success)
                            {
                                ...
                                response.Mensaje += " | " + ( _pspService.IsTestMode() 
                                    ? "?? SIMULACI�N: Archivos subidos exitosamente (modo prueba)"
        ...
        /// <summary>
        /// Sube archivos de validaci�n (DNI, selfie, inscripci�n AFIP, etc.)
        /// </summary>
        ...
        public async Task<IActionResult> GetProvinces([FromQuery] string uat)
        {
            try
            {
                ...
                var mensaje = _pspService.IsTestMode() 
                    ? "?? SIMULACI�N: Provincias obtenidas (modo prueba)"
        ...
        public async Task<IActionResult> GetCities([FromQuery] int provinceId, [FromQuery] string uat)
        {
            try
            {
                ...
                var mensaje = _pspService.IsTestMode() 
                    ? $"?? SIMULACI�N: Ciudades obtenidas para provincia {provinceId} (modo prueba)"
        ...
        // *** NUEVO ENDPOINT: CREAR ENTIDAD Y USUARIO EN UNA SOLA OPERACI�N ***
        /// <summary>
        /// Crea una entidad y usuario en el PSP en una sola operaci�n (endpoint /Entities/Persons/New)
        /// </summary>
        ...
        //    try
        //    {
        //        // Validar usuario autenticado usando el m�todo corregido
        //        var usuario = TraeUsuarioUAT(request.UAT);
        //        if (usuario == null)
        //        {
        ...
        /// <summary>
        /// Valida la configuraci�n del PSP
        /// </summary>
        ...
        public IActionResult ValidarConfiguracion([FromBody] PSPBaseResponseDTO request)
        {
            try
            {
                ...
                { 
                    Status = 200, 
                    UAT = request.UAT, 
                    Mensaje = isValid ? "Configuraci�n PSP v�lida" : "Configuraci�n PSP inv�lida",
                    Success = isValid
                });
            }
            catch (Exception ex)
        ...
        // *** NUEVO ENDPOINT: OBTENER INFORMACI�N DE CUENTAS ***
        /// <summary>
        ...
        public async Task<IActionResult> GetAccountsInfo([FromQuery] string userToken, [FromQuery] string uat)
        {
            try
            {
                ...
                var response = new AccountsInfoWithUATResponseDTO
                {
                    Status = pspResponse.Success ? 200 : 500,
                    UAT = uat,
                    Mensaje = pspResponse.Success ? "Informaci�n de cuentas obtenida exitosamente" : "Error al obtener informaci�n de cuentas",
                    Success = pspResponse.Success,
                ...
                if (pspResponse.Success)
                {
                    Log.Information($"Informaci�n de cuentas obtenida exitosamente - Total cuentas: {pspResponse.Accounts.Count}");
                    return Ok(response);
                }
                else
                {
                    Log.Warning($"Error al obtener informaci�n de cuentas: {pspResponse.Error}");
                    return BadRequest(response);
                }
            }
            catch (Exception ex)
